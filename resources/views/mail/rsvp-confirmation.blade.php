<div style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#f8f6f0;border:2px solid #1a2a3a;border-radius:10px;color:#1a2a3a;">
    ...
    <div style="font-size:13px;color:#6b7a8d;margin-bottom:20px;">New Event RSVP</div>

    <p style="font-size:15px;line-height:1.6;">
        Someone just RSVP'd to <strong>{{ $rsvp->event->title }}</strong>.
    </p>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:14px;margin-top:12px;">
        ...
        <tr>
            ...
            <td style="padding:8px 10px;border:1px solid #1a2a3a;">{{ $rsvp->visitor_name }} &lt;{{ $rsvp->visitor_email }}&gt;</td>
        </tr>
        <tr>
            <td style="padding:8px 10px;border:1px solid #1a2a3a;background:#eef3f7;">Status</td>
            <td style="padding:8px 10px;border:1px solid #1a2a3a;">{{ ucfirst($rsvp->status) }}</td>
        </tr>
    </table>

    <p style="font-size:13px;color:#6b7a8d;margin-top:20px;line-height:1.6;">
        Filled {{ $rsvp->event->current_rsvps }} of {{ $rsvp->event->max_capacity }} spots.
    </p>
</div>
