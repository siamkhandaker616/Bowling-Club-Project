<x-app-layout>
    ...
    <div class="mod-grid" style="min-height:calc(100vh - 200px);">
        ...
        <div style="background:var(--sky-light);border:2px solid var(--navy);border-radius:12px;padding:1rem;margin-bottom:1.25rem;">
            ...
            <form method="POST" action="{{ route('visitor.complaints.store') }}" class="gutter-form" style="display:grid;grid-template-columns:1fr 2fr auto;gap:8px;margin-top:8px;">
                @csrf
                ...
                <div class="gutter-field">
                    <input name="description" type="text" placeholder="What went wrong?" class="input">
            ...
            @forelse ($complaints as $complaint)
                <div style="background:var(--sky-light);border:2px solid var(--navy);border-radius:12px;padding:1rem;">
                    ...
                    @if ($complaint->resolution)
                        <div style="font-family:var(--font-mono);font-size:0.6rem;color:var(--sky-dark);margin-top:6px;">Resolution: {{ $complaint->resolution }}@if ($complaint->compensation_type) ({{ \App\Helpers\Label::compensationType($complaint->compensation_type) }})@endif</div>
                    @endif
                </div>
            @empty
